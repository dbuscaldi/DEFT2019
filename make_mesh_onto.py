...
franalyzer=LanguageAnalyzer("fr")
schema = Schema(name=TEXT(analyzer=franalyzer, stored=True), id=ID(stored=True), tags=TEXT(stored=True))
...
id=0
for c in onto:
    #print(str(id), c.name, c.tree_numbers)
    clean_name=re.sub("\[.+\]","",c.name)
    if c.tree_numbers != None:
        numbers=','.join(c.tree_numbers)
    else:
        numbers=""
    #print(clean_name, ','.join(c.tree_numbers))
    writer.add_document(name=u""+clean_name, id=u""+str(id), tags=u""+numbers)
    id+=1
